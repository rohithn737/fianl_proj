createcertificatesForOrg1() {
  ...
  #First we will register and then enroll
  #enrollment of the organization, CA is running on 7054 
  ...
  #for segrating the organizational units NodeOrganizational units (NodeOUS) are used
  #4 NodeOUS are used --> client,peer,admin,orderer
  echo 'NodeOUs:
  Enable: true
  ClientOUIdentifier:
  ...
  echo
  echo "Register peer0" #No peers at the start so creating peer0
  echo
  ...
  #User will have only enrollment certificates , services would have both enrollment and tls certificates
  echo
